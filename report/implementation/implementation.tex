...
Given a pulse signal, the heart rate corresponds to the average number of beats of the heart (seen as spikes in the signal) in a minute. A possible approach to identifying this is to count peaks in the signal. Alternatively, the heart rate could be taken as the prevalent frequency in the Fourier transform. The latter approach is used and the resulting transform provides a richer representation from which to identify the heart rate.
