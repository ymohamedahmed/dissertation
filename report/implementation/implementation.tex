...
the image since faces are not, in general, perfectly rectangular. As well as any imperfections in the face detector. \\ \\
These background pixels will not contain any information as to the underlying heart rate of the user.
As a result, considering the entire bounding box will add unecessary noise to the resulting signal which consists of the mean considered from each frame.
...
Furthermore,k it is unclear whether all skin pixels are equally useful. For example, it might be that cheeks contain greater predictive power than the nose.
...
Suppose that our observed colour signal, $\mathbf{I(t)}$, a vector-valued function, consists of a mixture of several underlying signals $\mathbf{x}(t)$ and the pulse signal $\mathbf{p}(t)$.
That is: $\mathbf{I}(t) = \lambda \mathbf{p}(t) + \sum_{i=1}^n \mu_i \mathbf{x_i}(t)$, for constants $\lambda$ and $\mu_i$. 
...
\\ \\
The above formulation is not enough to isolate the signal $\mathbf{p}(t)$. Thus assumptions must be placed on the nature of $\mathbf{x}(t)$ and $\mathbf{p}(t)$ in order to turn this into a tractable problem.
