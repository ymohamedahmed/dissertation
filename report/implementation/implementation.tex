...
\end{equation*}
\\ \\ 
\paragraph{Defining distributions}
Usually, one might attempt to learn both of the distributions required from data. That is the prior distribution $\Pr(C_\mathrm{skin}|x_i)$ and $\Pr(s|C_\mathrm{skin}, x_i)$.
...
the dataset discussed in Section \ref{section:colour-filter}. This dataset consists of colours randomly sampled which are labelled as either skin or not. As a result, a reasonable approach is to define the distribution $\Pr(s|C_\mathrm{skin}, x_i)$ based on 
